...
const weaponToRangeToDefenders0 = XMM.AgentUtilities.determineWeaponToRangeToDefenders(attackerInstance, gameState);
...
Object.keys(weaponToRangeToDefenders).forEach(weaponKey =>
{
   ...
   Object.keys(rangeToDefenders).forEach(rangeKey =>
   {
      ...
      weaponToRangeToDefenders = R.assocPath([weaponKey, rangeKey], newDefenders, weaponToRangeToDefenders);
   });
});
...
{
   console.log("myCallback() attackerId = " + attackerId + " weaponKey = " + weaponKey + " defenderId = " + defenderId);
};
