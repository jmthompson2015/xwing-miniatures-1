...
function processData(data0) {
  const { createData, determineData } = XWingConverter.OPTIONS;
  ...
  data.forEach(card => {
    ...
    if (enumProperties[key] !== undefined) {
      console.error(
        ...
        } ${enumProperties[key].faction} ${enumProperties[key].ship} ${enumProperties[key].slot}`
      );
      console.error(
...
XWingConverter.convert = (inputFile, className, options = XWingConverter.DEFAULT_OPTIONS) => {
