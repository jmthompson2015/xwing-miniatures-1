...
QUnit.test("ReferenceCard properties Auxiliary Firing Arc", assert => {
...
QUnit.test("keys()", assert => {
  EnumTest.keys(
    ...
    ReferenceCard,
    41,
    ReferenceCard.ACQUIRE_A_TARGET_LOCK_ACTION,
    ReferenceCard.USING_CLUSTER_MINES
  );
});

QUnit.test("required properties", assert => {
  EnumUtilities.values(ReferenceCard).forEach(reference => {
