...
const { PilotCard, UpgradeCard } = XMA;
...
QUnit.test("build() Core Set Imperial", assert => {
  ...
  const { pilotInstances, upgradeInstances } = store.getState();
...
QUnit.test("build2017USNationals1()", assert => {
  // Setup.
  const store = Redux.createStore(Reducer.root);
  const squadId = 1;
...
QUnit.test("build2013Worlds4()", assert => {
  // Setup.
  const store = Redux.createStore(Reducer.root);
  const squadId = 1;
...
QUnit.test("build2013Worlds5()", assert => {
  // Setup.
  const store = Redux.createStore(Reducer.root);
  const squadId = 1;
