...
const { AttackDiceValue, DefenseDiceValue, DiceModification, Token } = XMA;
...
const attackDiceFocusCount = state => {
  ...
  return combatId !== undefined
    ? Selector.attackDiceValueCount(combatId, AttackDiceValue.FOCUS, state)
    : 0;
};
...
const defenseDiceFocusCount = state => {
  const combatId = Selector.activeCombatId(state);
  return combatId !== undefined
    ? Selector.defenseDiceValueCount(combatId, DefenseDiceValue.FOCUS, state)
    : 0;
};
...
ModifyDiceAbility[DiceModification.ATTACK_SPEND_FOCUS] = {
  ...
  consequent: (attackerId, store) =>
    new Promise(resolve => {
      ...
      const newDiceKeys = DiceUtils.changeAllToValue(
        diceKeys,
