...
const { AttackDiceValue, DefenseDiceValue, Phase, Range } = XMA;
...
const verifyActiveCombatId = (assert, store, expected, messagePrefix = "callback ") => {
  ...
  assert.ok(
    XMS.Selector.combatInstance(activeCombatId, store.getState()),
    `${messagePrefix}combatInstance`
  );
};
...
const verifyAttackDiceKeys = (assert, store, expected, messagePrefix = "callback ") => {
  ...
  const { attackDiceKeys } = combatInstance;
...
const verifyDefenseDiceKeys = (assert, store, expected, messagePrefix = "callback ") => {
  const { activeCombatId } = store.getState();
  const combatInstance = store.getState().combatInstances[activeCombatId];
  const { defenseDiceKeys } = combatInstance;
...
const verifyCombatQueue = (assert, store, expected, messagePrefix = "callback ") => {
  const { combatQueue } = store.getState();
...
QUnit.test("doIt() Start", assert => {
  ...
  store0.dispatch(ActionCreator.setPhase(Phase.COMBAT_START));
  ...
  CombatTask.doIt(store0).then(callback);
});

QUnit.test("doIt() Declare Target query", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  setPhase(store0, Phase.COMBAT_DECLARE_TARGET);
  verifyActivePilotId(assert, store0, undefined, "");
...
QUnit.test("doIt() Declare Target response", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  ...
  setPhase(store0, Phase.COMBAT_DECLARE_TARGET);
  verifyActivePilotId(assert, store0, 3, "");
...
QUnit.test("doIt() Roll Attack Dice", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  addCombatInstance(store0);
  setPhase(store0, Phase.COMBAT_ROLL_ATTACK_DICE);
  verifyActivePilotId(assert, store0, 3, "");
...
QUnit.test("doIt() Modify Attack Dice query", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  addCombatInstance(store0);
  addAttackDice(store0);
  setPhase(store0, Phase.COMBAT_MODIFY_ATTACK_DICE);
  verifyActivePilotId(assert, store0, 3, "");
...
QUnit.test("doIt() Modify Attack Dice response", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
...
QUnit.test("doIt() Roll Defense Dice", assert => {
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  addCombatInstance(store0);
  addAttackDice(store0);
  setPhase(store0, Phase.COMBAT_ROLL_DEFENSE_DICE);
  verifyActivePilotId(assert, store0, 3, "");
...
QUnit.test("doIt() Modify Defense Dice query", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  addCombatInstance(store0);
  addAttackDice(store0);
  addDefenseDice(store0);
  setPhase(store0, Phase.COMBAT_MODIFY_DEFENSE_DICE);
  verifyActivePilotId(assert, store0, 3, "");
...
QUnit.test("doIt() Modify Defense Dice response", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
...
QUnit.test("doIt() Compare Results", assert => {
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  addCombatInstance(store0);
  addAttackDice(store0);
  addDefenseDice(store0);
  setPhase(store0, Phase.COMBAT_COMPARE_RESULTS);
  verifyActivePilotId(assert, store0, 3, "");
  ...
  const callback = store => {
    ...
    verifyAttackDiceKeys(assert, store, 4);
    verifyDefenseDiceKeys(assert, store, 3);
    const combatInstance = XMS.Selector.combatInstance(
      XMS.Selector.activeCombatId(store.getState()),
      store.getState()
    );
...
QUnit.test("doIt() Notify Damage query", assert => {
  // Setup.
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  ...
  addAttackDice(store0);
  addDefenseDice(store0);
  setPhase(store0, Phase.COMBAT_NOTIFY_DAMAGE);
  verifyActivePilotId(assert, store0, 3, "");
...
QUnit.test("doIt() Deal Damage", assert => {
  const store0 = initializeStore();
  store0.dispatch(ActionCreator.setCombatQueue([3, 1, 2]));
  store0.dispatch(ActionCreator.dequeueCombat());
  addCombatInstance(store0);
  addAttackDice(store0);
  addDefenseDice(store0);
  store0.dispatch(ActionCreator.setCombatShieldDamage(1, 0));
  store0.dispatch(ActionCreator.setCombatHitDamage(1, 0));
  store0.dispatch(ActionCreator.setCombatCriticalDamage(1, 1));
  setPhase(store0, Phase.COMBAT_DEAL_DAMAGE);
  verifyActivePilotId(assert, store0, 3, "");
  const combatInstance0 = XMS.Selector.combatInstance(
    XMS.Selector.activeCombatId(store0.getState()),
    store0.getState()
  );
  const defenderInstance0 = XMS.Selector.pilotInstance(
    combatInstance0.defenderId,
    store0.getState()
  );
  ...
  const callback = store => {
    ...
    assert.equal(combatInstance.hitDamage, 0, "callback combatInstance.hitDamage");
    assert.equal(combatInstance.criticalDamage, 1, "callback combatInstance.criticalDamage");
    const defenderInstance = XMS.Selector.pilotInstance(
      combatInstance.defenderId,
      store.getState()
    );
...
QUnit.test("doIt() End", assert => {
  // Setup.
  const store0 = initializeStore();
  setPhase(store0, Phase.COMBAT_END);
...
QUnit.test("doIt() Full Cycle", assert => {
  ...
  CombatTask.doIt(store0).then(callback01);
});
