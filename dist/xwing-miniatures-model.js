(function (global, factory) {
   ...
   Selector.agentInstanceByPilot = (pilotId, state) => XMS.Selector.agentInstanceByPilot(pilotId, state);

   Selector.agentInstanceBySquad = (squadId, state) => XMS.Selector.agentInstanceBySquad(squadId, state);
   ...
   Selector.attackDiceKeysByCombat = (combatId, state) => XMS.Selector.attackDiceKeysByCombat(combatId, state);

   Selector.attackDiceValueCount = (combatId, valueKey, state) => Selector.diceValueCount(valueKey)(Selector.attackDiceKeysByCombat(combatId, state));

   Selector.bonusByPilotStat = (pilotId, statKey, state) => R.defaultTo(0, XMS.Selector.bonusByPilotStat(pilotId, statKey, state));
   ...
   Selector.conditionCard = (conditionId, state) =>
   {
      ...
      return (conditionInstance !== undefined ? XMA.Selector.conditionCard(conditionInstance.conditionKey) : undefined);
   };

   Selector.conditionInstance = (conditionId, state) => XMS.Selector.conditionInstance(conditionId, state);

   Selector.countByPilotToken = (pilotId, tokenKey, state) => R.defaultTo(0, XMS.Selector.countByPilotToken(pilotId, tokenKey, state));

   Selector.countByUpgradeToken = (upgradeId, tokenKey, state) => R.defaultTo(0, XMS.Selector.countByUpgradeToken(upgradeId, tokenKey, state));

   Selector.damageCard = (damageId, state) => XMA.Selector.damageCard(Selector.damageInstance(damageId, state).damageKey);
   ...
   Selector.damageInstancesByPilot = (pilotId, state) => XMS.Selector.damageInstancesByPilot(pilotId, state);

   Selector.defenseDiceKeysByCombat = (combatId, state) => XMS.Selector.defenseDiceKeysByCombat(combatId, state);

   Selector.defenseDiceValueCount = (combatId, valueKey, state) => Selector.diceValueCount(valueKey)(Selector.defenseDiceKeysByCombat(combatId, state));

   Selector.diceValueCount = valueKey => diceKeys => R.reduce((accumulator, diceKey) =>
   {
      accumulator += (diceKey === valueKey ? 1 : 0);
      return accumulator;
   }, 0)(diceKeys);
   ...
   Selector.factionValueByPilot = (pilotId, state) => XMA.Selector.factionValueByPilot(Selector.pilotKey(pilotId, state));
   ...
   Selector.pilotInstancesByAgent = (agentId, state) => XMS.Selector.pilotInstancesByAgent(agentId, state);

   Selector.pilotInstancesBySquad = (squadId, state) => XMS.Selector.pilotInstancesBySquad(squadId, state);
   ...
   Selector.shieldByPilot = (pilotId, state) => Selector.countByPilotToken(pilotId, XMA.Token.SHIELD, state);

   Selector.shipBaseValueByPilot = (pilotId, state) => XMA.Selector.shipBaseValueByShip(Selector.shipByPilot(pilotId, state).key);

   Selector.shipByPilot = (pilotId, state) => XMA.Selector.shipValueByPilot(Selector.pilotKey(pilotId, state));
   ...
   Selector.targetLocksByAttacker = (attackerId, state) => XMS.Selector.targetLocksByAttacker(attackerId, state);

   Selector.targetLocksByDefender = (defenderId, state) => XMS.Selector.targetLocksByDefender(defenderId, state);

   Selector.upgradeCard = (upgradeId, state) => XMA.Selector.upgradeCard(Selector.upgradeKey(upgradeId, state));

   Selector.upgradeInstance = (upgradeId, state) => XMS.Selector.upgradeInstance(upgradeId, state);

   Selector.upgradeInstancesByPilot = (pilotId, state) => XMS.Selector.upgradeInstancesByPilot(pilotId, state);

   Selector.upgradeKey = (upgradeId, state) => R.prop("upgradeKey", Selector.upgradeInstance(upgradeId, state));

   Selector.upgradeKeysByPilot = (pilotId, state) => R.map(upgrade => upgrade.upgradeKey, Selector.upgradeInstancesByPilot(pilotId, state));

   Selector.weaponUpgradeIdsByPilot = (pilotId, state) => R.map(upgrade => upgrade.id, Selector.weaponUpgradeInstancesByPilot(pilotId, state));
   ...
   Selector.weaponUpgradeInstancesByPilot = (pilotId, state) =>
   {
      ...
      const filterFunction = instance => XMA.Selector.upgradeCard(instance.upgradeKey).attack !== undefined;
   ...
   DiceUtilities.addValue = function(diceKeys, newValue)
   {
   ...
   DiceUtilities.changeAllToValue = function(diceKeys, oldValue, newValue)
   {
      const newValues = Immutable.asMutable(diceKeys);
   ...
   DiceUtilities.computeAttackDiceCount = (activeCombatId, state) =>
   {
      ...
      const rangeKey = combatInstance.rangeKey;
   ...
   const AttackDiceValue$1 = XMA.AttackDiceValue;
   const DefenseDiceValue$1 = XMA.DefenseDiceValue;
   ...
   ModifyDiceAbility[DiceModification.ATTACK_SPEND_FOCUS] = {
      ...
      consequent: (attackerId, store) => new Promise((resolve) =>
      {
         ...
         const newDiceKeys = DiceUtilities.changeAllToValue(diceKeys, AttackDiceValue$1.FOCUS, AttackDiceValue$1.HIT);
   ...
   ModifyDiceAbility[DiceModification.DEFENSE_SPEND_FOCUS] = {
      ...
      consequent: (defenderId, store) => new Promise((resolve) =>
      {
         ...
         const combatId = Selector.activeCombatId(state);
         const diceKeys = Selector.defenseDiceKeysByCombat(combatId, state);
         const newDiceKeys = DiceUtilities.changeAllToValue(diceKeys, DefenseDiceValue$1.FOCUS, DefenseDiceValue$1.EVADE);
   ...
   {
      const combatId = Selector.activeCombatId(state);
      return (combatId !== undefined ? Selector.attackDiceValueCount(combatId, AttackDiceValue$1.FOCUS, state) : 0);
   };

   const defenseDiceFocusCount = state =>
   {
      const combatId = Selector.activeCombatId(state);
      return (combatId !== undefined ? Selector.defenseDiceValueCount(combatId, DefenseDiceValue$1.FOCUS, state) : 0);
   };
   ...
   const Token$1 = XMA.Token;

   const ActionCreator$1 = XMS.ActionCreator;
   ...
   const ActionCreator$2 = XMS.ActionCreator;
   ...
   AbilityUtilities.processAgentResponse = store =>
   {
      ...
      if (abilityState !== undefined)
      {
         ...
         {
            const pilotId = agentResponse.payload.pilotId;
   ...
   ManeuverComputer._interpolate = function(x0, y0, x1, y1, polygon1, shipBase)
   {
      ...
      if (((round$1(x0 - x01, 0) === 0) && (round$1(y0 - y01, 0) === 0)) ||
      ...
      else
      {
         ...
         const polygon01 = ManeuverComputer.computePolygon(shipBase, round$1(x01, 0), round$1(y01, 0), heading01);
   ...
   const round$1 = function(number, digits)
   {
      const factor = Math.pow(10.0, digits);
   ...
   const Token$2 = XMA.Token;

   const ActionCreator$3 = XMS.ActionCreator;
   ...
   PHASE_TO_CONFIG[Phase.ACTIVATION_EXECUTE_MANEUVER] = {
      processFunction: store =>
      {
         ...
         store.dispatch(ActionCreator$3.setDisplayManeuver(XMS.ManeuverState.create(
         {
            color: color,
   ...
   const AttackDiceValue$2 = XMA.AttackDiceValue;
   const DefenseDiceValue$2 = XMA.DefenseDiceValue;
   const Phase$1 = XMA.Phase;
   const Token$3 = XMA.Token;

   const ActionCreator$4 = XMS.ActionCreator;
   ...
   PHASE_TO_CONFIG$1[Phase$1.COMBAT_DECLARE_TARGET] = {
      processFunction: store =>
      {
         ...
         if (combatQueue.length > 0)
         {
            ...
            const weaponToRangeToDefenders = AgentUtilities.determineWeaponToRangeToDefenders(pilot, store.getState());
   ...
   PHASE_TO_CONFIG$1[Phase$1.COMBAT_COMPARE_RESULTS] = {
      processFunction: store =>
      {
         ...
         let criticals = Selector.attackDiceValueCount(combatId, AttackDiceValue$2.CRITICAL_HIT, store.getState());
   ...
   const Phase$2 = XMA.Phase;
   const Token$4 = XMA.Token;

   const ActionCreator$5 = XMS.ActionCreator;
   ...
   const Phase$3 = XMA.Phase;

   const ActionCreator$6 = XMS.ActionCreator;
      ...
      processFunction: store =>
      {
         ...
         if (planningQueue.length > 0)
         {
            ...
            const newAgentQuery = XMS.AgentQueryState.create(
            {
               ...
               payload:
               {
                  pilotToValidManeuvers: pilotToValidManeuvers
               }
            });
   ...
   const ActionCreator$7 = XMS.ActionCreator;
   ...
   const ActionCreator$8 = XMS.ActionCreator;
   ...
   SquadBuilder.build = function(store, name, year, description, squadId, pilotAndUpgradeKeys)
   {
   ...
   SquadBuilder.build2017USNationals1 = function(store, squadId)
   {
      ...
      const pilotAndUpgradeKeys = [
         {
            ...
            upgradeKeys: [UC.TWIN_LASER_TURRET, UC.EXTRA_MUNITIONS, UC.SABINE_WREN, UC.CLUSTER_MINES, UC.ION_BOMBS, UC.THERMAL_DETONATORS, UC.ADVANCED_SLAM]
         },
         {
            pilotKey: PC.CAPTAIN_NYM_REBEL_ALLIANCE,
            upgradeKeys: [UC.HAVOC, UC.VETERAN_INSTINCTS, UC.ADVANCED_SENSORS, UC.TWIN_LASER_TURRET, UC.BOMBLET_GENERATOR, UC.GENIUS, UC.ENGINE_UPGRADE]
         }];

      return SquadBuilder.build(store, name, year, description, squadId, pilotAndUpgradeKeys);
   };

   SquadBuilder.build2017USNationals2 = function(store, squadId)
   {
      ...
      const pilotAndUpgradeKeys = [
         ...
         {
            pilotKey: PC.MIRANDA_DONI,
            upgradeKeys: [UC.TWIN_LASER_TURRET, UC.CONCUSSION_MISSILES, UC.SABINE_WREN, UC.BOMBLET_GENERATOR, UC.LONG_RANGE_SCANNERS]
         },
         {
   ...
   SquadBuilder.build2017USNationals3 = function(store, squadId)
   {
   ...
   SquadBuilder.build2017USNationals4 = function(store, squadId)
   {
      ...
      const contractedScout1 = {
         ...
         upgradeKeys: [UC.ATTANNI_MINDLINK, UC.EXTRA_MUNITIONS, UC.PLASMA_TORPEDOES, UC.INTELLIGENCE_AGENT, UC.R4_AGROMECH, UC.GUIDANCE_CHIPS]
      };
      const pilotAndUpgradeKeys = [
         contractedScout1, contractedScout1,
         {
            pilotKey: PC.CONTRACTED_SCOUT,
            upgradeKeys: [UC.ATTANNI_MINDLINK, UC.PLASMA_TORPEDOES, UC.INTELLIGENCE_AGENT, UC.R4_AGROMECH, UC.GUIDANCE_CHIPS]
         }];

      return SquadBuilder.build(store, name, year, description, squadId, pilotAndUpgradeKeys);
   };

   SquadBuilder.build2017USNationals5 = function(store, squadId)
   {
   ...
   SquadBuilder.build2017Worlds2 = function(store, squadId)
   {
      const name = "Worlds #2";
      const year = 2017;
      ...
      const pilotAndUpgradeKeys = [
         {
            pilotKey: PC.MIRANDA_DONI,
            upgradeKeys: [UC.AUTOBLASTER_TURRET, UC.PLASMA_TORPEDOES, UC.EXTRA_MUNITIONS, UC.REY, UC.ION_BOMBS, UC.CLUSTER_MINES, UC.GUIDANCE_CHIPS],
         },
         {
            pilotKey: PC.BIGGS_DARKLIGHTER,
            upgradeKeys: [UC.R4_D6, UC.INTEGRATED_ASTROMECH]
   ...
   SquadBuilder.build2016Worlds3 = function(store, squadId)
   {
      const name = "Worlds #3";
      const year = 2016;
      const description = "K-Wing; T-70; X-Wing";
   ...
   SquadBuilder.build2013Worlds4 = function(store, squadId)
   {
      const name = "Worlds #4";
      const year = 2013;
      ...
      const pilotAndUpgradeKeys = [
         ...
         obsidianSquadronPilot, obsidianSquadronPilot, obsidianSquadronPilot, academyPilot, academyPilot];

      return SquadBuilder.build(store, name, year, description, squadId, pilotAndUpgradeKeys);
   };

   SquadBuilder.build2013Worlds5 = function(store, squadId)
   {
      const name = "Worlds #5";
      const year = 2013;
      ...
      const pilotAndUpgradeKeys = [
         ...
         rookiePilot, rookiePilot, goldSquadronPilot, goldSquadronPilot];

      return SquadBuilder.build(store, name, year, description, squadId, pilotAndUpgradeKeys);
   };

   SquadBuilder.buildCoreSetImperial = function(store, squadId)
   {
   ...
   SquadBuilder.buildCoreSetRebel = function(store, squadId)
   {
   ...
   {
      const pilotKey = pilotObj.pilotKey;
      ...
      const tokenCounts = R.assoc("shield", PilotUtils.statValue("shields", pilotKey), XMS.TokenCountsState.create());
   ...
   const ActionCreator$9 = XMS.ActionCreator;
   ...
   exports.PilotUtilities = PilotUtils;
